...
class CsvExportService {
  ...
  Future<CsvExportResult> exportData(DateTime? startDate) async {
    try {
      ...
      final dailyResult = await _exportDailyLogs(
        isar,
        normalizedStart,
        normalizedEnd,
      );
      final exerciseResult = await _exportExerciseLogs(
        isar,
        normalizedStart,
        normalizedEnd,
      );
      final habitsResult = await _exportHabitCompletions(
        isar,
        normalizedStart,
        normalizedEnd,
      );
      final bodyStatsResult = await _exportBodyStats(
        isar,
        normalizedStart,
        normalizedEnd,
      );
      final mealsResult = await _exportMeals(
        isar,
        normalizedStart,
        normalizedEnd,
      );
      ...
      final fileName =
          'trufit_export_${startSnapshot.millisecondsSinceEpoch}.zip';
            ...
            try {
              itemRows.add(
                [
                  ...
                  item.computedNutrition?.proteinG,
                  item.computedNutrition?.carbsG,
                  item.computedNutrition?.fatG,
                  item.provenance,
                  item.resolved,
                ].map(_sanitizeForCsv).toList(),
              );
